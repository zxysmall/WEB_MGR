package com.xy.modular.system.controller;

import com.xy.common.annotion.BussinessLog;
import com.xy.common.annotion.Permission;
import com.xy.common.constant.Const;
import com.xy.common.constant.dictmap.UserDict;
...
import com.xy.core.db.Db;
import com.xy.core.exception.XyException;
import com.xy.core.log.LogObjectHolder;
import com.xy.core.shiro.ShiroKit;
import com.xy.core.shiro.ShiroUser;
import com.xy.core.util.ToolUtil;
import com.xy.modular.system.dao.UserMgrDao;
import com.xy.modular.system.factory.UserFactory;
import com.xy.modular.system.transfer.UserDto;
import com.xy.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    public String roleAssign(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new XyException(BizExceptionEnum.REQUEST_NULL);
        }
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
        if (!newPwd.equals(rePwd)) {
            throw new XyException(BizExceptionEnum.TWO_PWD_NOT_MATCH);
        }
        Integer userId = ShiroKit.getUser().getId();
        ...
        if (user.getPassword().equals(oldMd5)) {
            ...
            throw new XyException(BizExceptionEnum.OLD_PWD_NOT_RIGHT);
        }
    }
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
        ...
        if (theUser != null) {
            throw new XyException(BizExceptionEnum.USER_ALREADY_REG);
        }
    ...
    public Tip edit(@Valid UserDto user, BindingResult result) throws NoPermissionException {
        ...
        if (ShiroKit.hasRole(Const.ADMIN_NAME)) {
            ...
            if (shiroUser.getId().equals(user.getId())) {
                ...
                throw new XyException(BizExceptionEnum.NO_PERMITION);
            }
        }
    }
    ...
    public Tip delete(@RequestParam Integer userId) {
        ...
        if (userId.equals(Const.ADMIN_ID)) {
            throw new XyException(BizExceptionEnum.CANT_DELETE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip freeze(@RequestParam Integer userId) {
        ...
        //不能冻结超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new XyException(BizExceptionEnum.CANT_FREEZE_ADMIN);
        }
        assertAuth(userId);
    ...
    public Tip setRole(@RequestParam("userId") Integer userId, @RequestParam("roleIds") String roleIds) {
        ...
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new XyException(BizExceptionEnum.CANT_CHANGE_ADMIN);
        }
        assertAuth(userId);
    ...
    public String upload(@RequestPart("file") MultipartFile picture) {
        ...
        try {
            String fileSavePath = xyProperties.getFileUploadPath();
            picture.transferTo(new File(fileSavePath + pictureName));
        } catch (Exception e) {
            throw new XyException(BizExceptionEnum.UPLOAD_ERROR);
        }
        return pictureName;
    }
